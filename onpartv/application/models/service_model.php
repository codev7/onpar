...
//		var_dump($new_order);
	...
	// Get scheduling for users function by Kevin B on 11-23-15
	public function viewSchedule($user_id) {
		$query_string = "SELECT file_name, name_only, title, origin_name, display_order, begin_date, end_date, stay_time FROM " . $this->FILE_TABLE;
		$query_string .= " WHERE user_id = " . $user_id;
		$query_string .= " ORDER BY `display_order`";
		$query = $this->db->query($query_string);
		$result = $query->result_array();
